...
MANIFEST = "top200_citers_manifest.csv"
OUTDIR = Path("data/arxiv_pdfs/25808_citers")
...
def clean_arxiv_id(x: str) -> str:
    ...
    if "v" in x and x.rsplit("v", 1)[-1].isdigit():
        x = x.rsplit("v", 1)[0]
    return x
...
with open(LOGDIR / "arxiv_citers_success.csv", "w", newline="", encoding="utf-8") as f:
    w = csv.writer(f)
    w.writerow(["cn", "arxiv_id", "status"])
    w.writerows(SUCCESS)

with open(LOGDIR / "arxiv_citers_fail.csv", "w", newline="", encoding="utf-8") as f:
    w = csv.writer(f)
...
print(f"Logs: {LOGDIR/'arxiv_citers_success.csv'} and {LOGDIR/'arxiv_citers_fail.csv'}")
